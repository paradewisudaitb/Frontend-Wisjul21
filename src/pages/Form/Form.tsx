...
const schema = yup.object().shape({
  ...
  'showAtWeb': yup.bool().default(() => false),
});
...
export default function Form(): JSX.Element {
  ...
  const submitForm = async (data: any) => {
    ...
    const cleanAndSplit = (d: string): string[] => {
      return d.split('\n').filter((e: string) => e != '').map((e: string) => e.trim());
    };
    ...
    const req: IDataWisudawan = {
      ...
      lembaga: cleanAndSplit(data.nonhmj),
      kontribusi: cleanAndSplit(data.kontribusi),
      prestasi: cleanAndSplit(data.prestasi),
      karya: cleanAndSplit(data.karya),
    };
  ...
  const now = Date.now();
  const timeStart = new Date('2021-07-10T08:00').getTime();
  const timeEnd = new Date('2021-07-12T08:00').getTime();
  const enabled = timeStart <= now && now <= timeEnd;
        ...
        <div className="form-container">
          <form className="form" onSubmit={handleSubmit(submitForm)}>
            ...
            <div className="form-content">
              ...
              <Row>
                <Col className="form-nopadding">
                  <div className="d-flex justify-content-between">
                    <select className="form-select form-field" defaultValue="" required {...register('himpunan')}>
                      <option className="form-select-name" value="" disabled> Himpunan </option>
                      ...
                      <option className="form-select-option" value="Keluarga Mahasiswa Teknik Penerbangan"> KMPN (Keluarga Mahasiswa Teknik Penerbangan) </option>
                    ...
                    <select className="form-select form-field" id="pilihanJurusan" defaultValue="" required {...register('jurusan')}>
                      <option className="form-select-name" value="" disabled> Jurusan </option>
                      {jurusanOption}
                    </select>
                  </div>
              ...
              <Row>
                <Col>
                  <input className="form-checkbox" type="checkbox" {...register('showAtWeb')} />
                </Col>
                <Col xs={11}>
                  <label htmlFor="showAtWeb">
                    Saya ingin data saya ditunjukkan pada website Parade Wisuda Juli ITB 2021
                  </label>
                </Col>
              </Row>
              <Row>
              ...
              <Row>
                ...
                <input
                  ...
                  required {...register('tanggallahir')}
                />
              </Row>
              <Row>
                <label htmlFor="angkatan">Angkatan</label>
                <select className="form-select form-field" defaultValue="" required {...register('angkatan')}>
                  <option className="form-select-name" value="" disabled> Angkatan </option>
